    ...
    <main className="min-h-screen bg-[#f7f9fc] text-[#0b1220]">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6 lg:grid-cols-[220px_1fr] lg:px-8 lg:py-8">
        ...
        <section className="min-w-0 space-y-6">
          ...
          <div id="overview" className="rounded-[2rem] border border-[#dbe6f7] bg-gradient-to-br from-[#eef4ff] via-white to-white p-7 shadow-sm sm:p-10"><p className="text-sm font-bold uppercase tracking-[0.16em] text-[#155eef]">Your workspace</p><h1 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Welcome to RemoteScout.</h1><p className="mt-4 max-w-2xl leading-7 text-[#667085]">Your account is ready. Discover remote opportunities, save the ones you like, and keep track of applications as the product grows.</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/jobs" className="rounded-full bg-[#155eef] px-5 py-3 text-sm font-bold text-white shadow-lg shadow-[#155eef]/20 hover:bg-[#0b4dcc]">Browse remote jobs</Link><Link href="/dashboard/saved" className="rounded-full border border-[#d8e0ea] bg-white px-5 py-3 text-sm font-bold hover:border-[#155eef] hover:text-[#155eef]">View saved jobs</Link></div></div>

          <div className="grid gap-4 sm:grid-cols-3">
            {[ ["Job discovery", "Available", "Browse active remote opportunities from connected sources."], ["Saved jobs", "Available", "Save interesting roles and return to them later."], ["Applications", "Coming soon", "Keep a simple record of the roles you decide to apply for."] ].map(([title, status, text]) => <article key={title} className="rounded-2xl border border-[#e4e9f0] bg-white p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-[#155eef]">{status}</p><h2 className="mt-3 font-extrabold">{title}</h2><p className="mt-2 text-sm leading-6 text-[#667085]">{text}</p>{title === "Saved jobs" ? <Link href="/dashboard/saved" className="mt-4 inline-block text-sm font-bold text-[#155eef] hover:underline">Open saved jobs →</Link> : null}</article>)}
          </div>
