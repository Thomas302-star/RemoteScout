    ...
    <main className="min-h-screen overflow-x-hidden bg-[#f7f9fc] text-[#0b1220]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-5 sm:py-8 lg:px-8 lg:py-10">
        ...
        <section className="mb-6 rounded-2xl border border-[#e1e7ef] bg-white p-4 shadow-sm sm:mb-7 sm:p-6">
          <form action="/jobs" method="get" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] lg:items-end">
            <label className="min-w-0 sm:col-span-2 lg:col-span-1"><span className="mb-2 block text-sm font-bold text-[#344054]">Search jobs</span><input name="q" defaultValue={q} placeholder="Title, company, location..." className="w-full min-w-0 rounded-xl border border-[#d8e0ea] px-4 py-3 text-sm outline-none focus:border-[#155eef] focus:ring-4 focus:ring-[#155eef]/10" /></label>
            ...
            <div className="flex w-full gap-2 sm:col-span-2 lg:col-span-1"><button type="submit" className="min-w-0 flex-1 rounded-xl bg-[#155eef] px-5 py-3 text-sm font-bold text-white hover:bg-[#0b4dcc]">Search</button>{hasFilters ? <Link href="/jobs" className="shrink-0 rounded-xl border border-[#d8e0ea] px-4 py-3 text-sm font-bold">Clear</Link> : null}</div>
          </form>
        </section>
          ...
          <div className="grid gap-4 lg:grid-cols-2 lg:gap-5">
            {typedJobs.map((job) => {
              ...
              return <article key={job.id} className="group min-w-0 rounded-2xl border border-[#e1e7ef] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[#bfd0ee] hover:shadow-md sm:p-6">
                ...
                <div className="mt-3 flex flex-col gap-2 border-t border-[#eef1f5] pt-3 sm:flex-row"><a href={job.original_job_url} target="_blank" rel="noreferrer" className="inline-flex min-w-0 flex-1 items-center justify-center rounded-xl border border-[#d8e0ea] px-4 py-2.5 text-center text-sm font-bold text-[#475467] hover:border-[#155eef] hover:text-[#155eef]">View source</a><a href={job.application_url} target="_blank" rel="noreferrer" className="inline-flex min-w-0 flex-1 items-center justify-center rounded-xl bg-[#155eef] px-4 py-2.5 text-center text-sm font-bold text-white hover:bg-[#0b4dcc]">Apply for this job</a></div>
              </article>;
            })}
          </div>
        )}
