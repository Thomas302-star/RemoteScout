...
export default function DashboardShell({ email, emailVerified = false, isAdmin = false }: { email: string; emailVerified?: boolean; isAdmin?: boolean }) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fc] text-[#0b1220]">
      <header className="sticky top-0 z-20 border-b border-[#e4e9f0] bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8"><Link href="/" className="flex items-center gap-2.5"><span className="grid size-9 place-items-center rounded-xl bg-[#155eef] text-sm font-black text-white">R</span><span className="text-lg font-extrabold tracking-tight">RemoteScout</span></Link><div className="flex items-center gap-3"><Link href="/dashboard/profile" className="hidden max-w-52 truncate text-sm font-semibold text-[#667085] hover:text-[#155eef] sm:block">{email}</Link><button type="button" onClick={signOut} disabled={loading} className="rounded-full border border-[#d8e0ea] bg-white px-4 py-2.5 text-sm font-semibold hover:border-[#155eef] hover:text-[#155eef] disabled:opacity-60">{loading ? "Signing out..." : "Sign out"}</button></div></div></header>
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6 lg:grid-cols-[220px_1fr] lg:px-8 lg:py-8">
        <aside className="hidden lg:block"><nav className="sticky top-24 rounded-2xl border border-[#e1e7ef] bg-white p-3 shadow-sm"><p className="px-3 pb-2 text-xs font-bold uppercase tracking-[0.14em] text-[#98a2b3]">Workspace</p><div className="space-y-1">{dashboardItems.map((item, index) => <Link key={item.label} href={item.href} className={`block rounded-xl px-3 py-2.5 text-sm font-semibold transition ${index === 0 ? "bg-[#eff4ff] text-[#155eef]" : "text-[#667085] hover:bg-[#f8fafc] hover:text-[#0b1220]"}`}>{item.label}</Link>)}{isAdmin ? <Link href="/admin/jobs" className="mt-3 block rounded-xl bg-[#0b1220] px-3 py-2.5 text-sm font-bold text-white hover:bg-[#1d2939]">Admin jobs</Link> : null}</div></nav></aside>
