          ...
          <nav className="sticky top-24 rounded-2xl border border-[#e1e7ef] bg-white p-3 shadow-sm">
            ...
            <div className="space-y-1">
              {dashboardItems.map((item, index) => (
                <Link key={item.label} href={item.href} className={`block rounded-xl px-3 py-2.5 text-sm font-semibold transition ${index === 0 ? "bg-[#eff4ff] text-[#155eef]" : "text-[#667085] hover:bg-[#f8fafc] hover:text-[#0b1220]"}`}>
                  {item.label}
                </Link>
              ))}
              {isAdmin ? (
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[["Job discovery", "Available", "Browse active remote opportunities from connected sources."], ["Saved jobs", "Available", "Save interesting roles and return to them later."], ["Applications", "Available", "Track roles you applied to and update their status."]].map(([title, status, text]) => (
              <article key={title} className="rounded-2xl border border-[#e4e9f0] bg-white p-5 shadow-sm">
                ...
                {title === "Saved jobs" ? <Link href="/dashboard/saved" className="mt-4 inline-block text-sm font-bold text-[#155eef] hover:underline">Open saved jobs →</Link> : null}
                {title === "Applications" ? <Link href="/dashboard/applications" className="mt-4 inline-block text-sm font-bold text-[#155eef] hover:underline">Open applications →</Link> : null}
              </article>
            ))}
          </div>
